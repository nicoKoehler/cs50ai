...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        sentence_add = Sentence({cell}, 0)

        if sentence_add not in self.knowledge: 
            self.knowledge.append(sentence_add)
        ...
        #clean list
        knowledge_buffer = []
        [knowledge_buffer.append(x) for x in self.knowledge if x not in knowledge_buffer]

        self.knowledge = knowledge_buffer
        #post-hoc add
        for sen in self.knowledge:
            mines_new = sen.known_mines()
            safes_new = sen.known_safes()
    ...
    def show_knowledge(self, knowledge=[]):
        ...
        for i,k in enumerate(knowledge_sorted):
            print(f"{i}: {{{k}}}")
